...
bool check_pit_collision(Player* player) {
    float pit_threshold = (LEVEL_HEIGHT - 1) * TILE_SIZE + 50;
    ...
    if (player->velocity.x > 0) {
        ...
        for (float y = top_y; y < bottom_y; y += TILE_SIZE/4) {
            ...
            if (tile != TILE_EMPTY) {
                ...
                player->rectangle.x = (tile_x + 1) * TILE_SIZE + 1;
                return true;
            }
        }
    }
...
bool check_player_ground_collision(Player* player, Level level) {
    float bottom_y = player->rectangle.y + player->rectangle.height + 1;
